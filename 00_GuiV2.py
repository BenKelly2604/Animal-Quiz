...
RESULT_FONT = ("Helvetica", 24, "bold")
LABEL_BG_COLOR = "white"

# Global variables to store the windows stack, main window closed flag, and quiz results
window_stack = []
quiz_results = []
...
animal_list = sorted(["Alpaca", "Ant", "Bat", "Bee", "Crocodile", "Deer", "Cheetah", "Duck",
                      ...
                      "Pig",
                      "Penguin", "Platypus", "Pelican", "Orangutan", "Polar Bear", "Rabbit", "Sloth", "Seagull",
                      "Red Panda",
                      "Snake", "Sheep", "Squirrel", "Turkey", "Wolf", "Wombat"])
animal_descriptions = {
...
animal_descriptions_quiz = {
    "bear": "What range of physical appearances and colors are exhibited, and how do these vary depending on species and habitats?",

    "cat": "In what ways do the shapes, sizes, and colors adapt to the environment and lifestyle?",

    "dog": "How does the diversity in physical appearances, sizes, and coat colors reflect the wide range?",

    "sheep": "What distinct physical characteristics and behaviors suit the grazing lifestyle?",

    "alpaca": "What are the unique physical features, and why is the fleece considered valuable?",

    "ant": "What makes these insects fascinating in terms of their highly organized social structures and impressive abilities?",

    "bat": "How do elongated finger bones and leathery wing membranes enable the ability to fly?",

    "bee": "What are the unique physical adaptations, and why are they vital in pollination?",

    "crocodile": "What are the distinctive physical appearances and formidable traits?",

    "deer": "What physical traits contribute to grace, and what are the main features?",

    "cheetah": "What unique physical appearance and abilities contribute to incredible speed?",

    "duck": "What distinctive physical features and behaviors are observed?",

    "crab": "What unique adaptations exist for life in aquatic environments?",

    "spider": "What are the intriguing physical traits, including silk-producing abilities?",

    "rhino": "What distinctive physical feature is known, and what role does it play in the herbivorous lifestyle?",

    "giraffe": "What are the key physical characteristics that make this mammal iconic?",

    "elephant": "What physical traits are known for majesty, and how do the trunk and tusks function?",

    "tiger": "What physical characteristics and behaviors are known, especially in terms of hunting?",

    "lion": "What physical and social traits contribute to renown among big cats?",

    "zebra": "What physical feature is easily recognizable, and how does it benefit them?",

    "kangaroo": "What unique locomotion and physical features are characteristic?",

    "buffalo": "What physical traits and habitats are known, particularly in the African variety?",

    "beaver": "What unique behaviors and physical traits contribute to impressive engineering skills?",

    "donkey": "What physical characteristics and abilities are known for hardiness and endurance?",

    "tasmanian devil": "What unique traits and behaviors are observed?",

    "hamster": "What physical features and behaviors are observed, especially in terms of food storage?",

    "hawk": "What physical traits contribute to effectiveness as a predator?",

    "fox": "What physical and behavioral traits contribute to their cunning nature?",

    "hedgehog": "What defensive mechanism is unique?",

    "horse": "What physical characteristics and historical significance contribute to importance?",

    "kiwi": "What unique physical traits and behaviors are characteristic of this bird?",

    "lizard": "What physical traits and abilities contribute to adaptability to different environments?",

    "leopard": "What physical traits and behaviors contribute to stealthy hunting?",

    "koala": "What physical and behavioral traits are iconic?",

    "lemur": "What unique physical and behavioral traits are observed, especially in Madagascar?",

    "mole": "What adaptations are well-suited for a subterranean lifestyle?",

    "panther": "What defines this term in terms of physical appearance among big cats?",

    "pig": "What physical traits and behaviors contribute to intelligence and social nature?",

    "penguin": "What physical adaptations contribute to suitability for aquatic environments?",

    "platypus": "What unique physical traits define this animal?",

    "pelican": "What physical traits and behaviors are observed?",

    "orangutan": "What physical and behavioral traits are characteristic?",

    "polar bear": "What physical traits and behaviors contribute to the status as iconic apex predators?",

    "rabbit": "What physical traits and behaviors are observed, especially in terms of locomotion?",

    "sloth": "What physical traits and behaviors characterize this slow-moving animal?",

    "seagull": "What physical traits and behaviors contribute to adaptability to different environments?",

    "red panda": "What unique physical traits and behaviors are characteristic?",

    "snake": "What physical traits and behaviors are observed in this legless reptile?",

    "squirrel": "What physical traits and behaviors contribute to food storage practices?",

    "turkey": "What physical traits and behaviors make this bird distinctive?",

    "wolf": "What physical and social traits contribute to iconic status as carnivores?",

    "wombat": "What physical traits and behaviors are characteristic of this burrowing marsupial?"
}
difficulty_path = {
    "easy": "Images/easy/",
    "challenging": "Images/medium/",
    "hard": "Images/hard/"
}
...
def randomize_options(question):
    """Randomizes the options for a multiple-choice question."""
...
def resize_image(image, max_width, max_height):
    """Resizes the given image to fit within the specified width and height."""
    w, h = image.size
...
def validate_answer(answer):
    """Validates if the given answer is a valid animal name."""
...
def load_mc_question(quiz_window, idx, score, questions):
    """Loads a multiple-choice question in the quiz window."""
    for widget in quiz_window.winfo_children():
        widget.destroy()

    question = questions[idx]
    question_label = tk.Label(quiz_window, text=f"Question {idx + 1}: {question['question']}", font=QUESTION_FONT, bg=LABEL_BG_COLOR, wraplength=550, justify=tk.LEFT)
    question_label.pack(pady=10)

    options_frame = tk.Frame(quiz_window, bg=LABEL_BG_COLOR)
    options_frame.pack(pady=10)

    options = randomize_options(question)
    for i, option in enumerate(options):
        option_label = tk.Label(options_frame, text=f"{chr(65 + i)}. {option}", font=TEXT_FONT, width=20, height=2, bg="lightgray", relief="groove")
        option_label.grid(row=i//2, column=i % 2, padx=5, pady=5)

    feedback_label = tk.Label(quiz_window, text="", font=TEXT_FONT, bg=LABEL_BG_COLOR)
    feedback_label.pack(pady=5)

    answer_entry = tk.Entry(quiz_window, font=TEXT_FONT, width=40)
    answer_entry.pack(pady=5)

    submit_button = tk.Button(quiz_window, text="Submit", font=TEXT_FONT,
...
def display_next_mc_question(quiz_window, idx, score, questions, answer_entry, feedback_label, submit_button):
    """Displays the next multiple-choice question or results if quiz is finished."""
    ...
    if answer.upper() in option_letters:
        selected_option = options[ord(answer.upper()) - 65]  # Convert letter to corresponding option
    elif answer.lower() in [option.lower() for option in options]:
        selected_option = answer
    else:
        feedback_label.config(text="Please enter a valid option (A, B, C, D) or a valid animal name.", fg="red")
        return
    ...
    if idx < len(questions) - 1:
        ...
        submit_button.config(text="Show Results",
                             command=lambda: show_results(quiz_window, score, len(questions)))
...
def show_results(quiz_window, score, questions):
    ...
    result_label = tk.Label(quiz_window, text=f"Your Score: {score}/{len(questions)}", font=RESULT_FONT, bg=LABEL_BG_COLOR)
    result_label.pack(pady=20)

    close_button = tk.Button(quiz_window, text="Close", command=quiz_window.destroy, font=TEXT_FONT)
...
def create_topics_window(title, topics):
    """Creates a new window to display topics."""
    global window_stack
    ...
    new_window.configure(bg=LABEL_BG_COLOR)

    title_label = tk.Label(new_window, text=title, font=TITLE_FONT, bg=LABEL_BG_COLOR)
    title_label.pack(pady=20)

    for topic in topics:
        topic_label = tk.Label(new_window, text=topic, font=TEXT_FONT, bg=LABEL_BG_COLOR)
        topic_label.pack()
...
def close_window(window_to_close=None):
    """Closes the current window and shows the previous one."""
    global window_stack
    if window_stack:
...
def close_previous_window():
    """Closes the previous window if it exists."""
    if window_stack:
...
# Animal Details Functions


def show_animal_details(animal):
    """Displays details about the selected animal."""
    detail_window = tk.Toplevel()
    detail_window.title(animal.capitalize())
    detail_window.geometry(WINDOW_SIZE)
    detail_window.configure(bg=LABEL_BG_COLOR)

    canvas = tk.Canvas(detail_window, bg=LABEL_BG_COLOR)
    ...
    detail_frame = tk.Frame(canvas, bg=LABEL_BG_COLOR)
    canvas.create_window((0, 0), window=detail_frame, anchor='nw')

    title_label = tk.Label(detail_frame, text=animal.capitalize(), font=TITLE_FONT, bg=LABEL_BG_COLOR)
    title_label.pack(pady=20)
    ...
    try:
        ...
        image_label = tk.Label(detail_frame, image=animal_photo, bg=LABEL_BG_COLOR)
        image_label.image = animal_photo  # Keep a reference to avoid garbage collection
        image_label.pack(pady=20)
    except FileNotFoundError:
        image_label = tk.Label(detail_frame, text="Image not available", font=TEXT_FONT, bg=LABEL_BG_COLOR)
        image_label.pack(pady=20)

    description = animal_descriptions.get(animal.lower(), "Description not available.")
    description_label = tk.Label(detail_frame, text=description, font=TEXT_FONT, wraplength=760, justify="left", bg=LABEL_BG_COLOR)
...
def search_animal(event, search_entry, search_listbox):
...
# Learning and Quiz Functions
    """Opens the learning window."""
...
def open_learn():
    ...
    learn_window.title("Learn about Animals")
    learn_window.geometry(WINDOW_SIZE)
    learn_window.configure(bg=LABEL_BG_COLOR)

    title_label = tk.Label(learn_window, text="Learn about Animals", font=TITLE_FONT, bg=LABEL_BG_COLOR)
    title_label.pack(pady=20)

    search_frame = tk.Frame(learn_window, bg=LABEL_BG_COLOR)
    search_frame.pack(pady=20)

    search_label = tk.Label(search_frame, text="Search Animal:", font=TEXT_FONT, bg=LABEL_BG_COLOR)
    search_label.pack(side=tk.LEFT, padx=5)

    search_entry = tk.Entry(search_frame, font=TEXT_FONT)
    search_entry.pack(side=tk.LEFT, padx=5)

    search_button = tk.Button(search_frame, text="Search", font=TEXT_FONT, command=lambda: perform_search(search_entry, search_listbox))
    search_button.pack(side=tk.LEFT, padx=5)

    search_listbox = tk.Listbox(learn_window, font=TEXT_FONT)
    search_listbox.pack(pady=10, fill=tk.BOTH, expand=True)

    search_entry.bind("<KeyRelease>", lambda event: search_animal(event, search_entry, search_listbox))

    # Display the full animal list initially
    for animal in animal_list:
        search_listbox.insert(tk.END, animal)

    search_listbox.bind('<<ListboxSelect>>', lambda event: show_animal_details(search_listbox.get(search_listbox.curselection())))

    learn_window.protocol("WM_DELETE_WINDOW", lambda: close_window(learn_window))
    window_stack.append(learn_window)


def perform_search(search_entry, search_listbox):
    """Performs the search for the animal based on the user's input."""
    search_term = search_entry.get().strip().title()
    if search_term in animal_list:
        search_listbox.delete(0, tk.END)
        search_listbox.insert(tk.END, search_term)
        show_animal_details(search_term)
    else:
        messagebox.showerror("Error", "Please input a valid animal from the list.")
...
def display_next_question(quiz_window, idx, score, correct_answers, answer_entry, feedback_label, difficulty, quiz_type):
    ...
    if validate_answer(answer):
        ...
        submit_button.config(text="Next Question",
                             command=lambda: load_question(quiz_window, idx + 1, score, correct_answers, difficulty, quiz_type) if idx < len(correct_answers) - 1 else show_results(quiz_window, score, quiz_type))
    else:
...
def load_question(quiz_window, idx, score, correct_answers, difficulty, quiz_type):
    """Loads the next question in the quiz window."""
    for widget in quiz_window.winfo_children():
        widget.destroy()

    if quiz_type == "Whose This?":
        question_label = tk.Label(quiz_window, text=f"Question {idx + 1}: What is this animal?", font=QUESTION_FONT, bg=LABEL_BG_COLOR)
        question_label.pack(pady=20)
    else:
        question_label = tk.Label(quiz_window, text=f"Question {idx + 1}: {animal_descriptions_quiz[correct_answers[idx].lower()]}", font=QUESTION_FONT, bg=LABEL_BG_COLOR)
        question_label.pack(pady=20)
    ...
    try:
        ...
        image_label = tk.Label(quiz_window, image=animal_photo, bg=LABEL_BG_COLOR)
        image_label.image = animal_photo  # Keep a reference to avoid garbage collection
        image_label.pack(pady=20)
    except FileNotFoundError:
        image_label = tk.Label(quiz_window, text="Image not available", font=TEXT_FONT, bg=LABEL_BG_COLOR)
        image_label.pack(pady=20)

    answer_entry = tk.Entry(quiz_window, font=TEXT_FONT)
    ...
    submit_button = tk.Button(quiz_window, text="Submit", font=TEXT_FONT,
                              command=lambda: display_next_question(quiz_window, idx, score, correct_answers, answer_entry, feedback_label, difficulty, quiz_type))
    submit_button.pack(pady=20)

    # Add a suggestion list for animal name suggestions
    suggestions_frame = tk.Frame(quiz_window, bg=LABEL_BG_COLOR)
...
def update_suggestions(event, entry, suggestions_listbox, correct_answers):
    """Updates the suggestions listbox based on the user's input."""
...
def show_results(quiz_window, score, quiz_type):
    ...
    result_label = tk.Label(quiz_window, text=f"Your Score: {score}/10", font=RESULT_FONT, bg=LABEL_BG_COLOR)
    result_label.pack(pady=50)

    close_button = tk.Button(quiz_window, text="Close", command=lambda: close_window(quiz_window), font=TEXT_FONT)
    close_button.pack(pady=20)

    # Enable history button if at least one quiz is completed
...
def open_quiz():
    ...
    quiz_window.title("Animal Quiz")
    quiz_window.geometry("400x400")
    quiz_window.configure(bg=LABEL_BG_COLOR)

    title_label = tk.Label(quiz_window, text="Choose a Quiz", font=TITLE_FONT, bg=LABEL_BG_COLOR)
    title_label.pack(pady=20)

    whose_this_button = tk.Button(quiz_window, text="Whose This?", width=20, height=2, command=lambda: open_difficulty_selection("Whose This?"))
    whose_this_button['font'] = BUTTON_FONT
    whose_this_button.pack(pady=20)

    who_can_do_that_button = tk.Button(quiz_window, text="Who Can Do That?", width=20, height=2, command=lambda: open_difficulty_selection("Who Can Do That?"))
    who_can_do_that_button['font'] = BUTTON_FONT
    who_can_do_that_button.pack(pady=20)

    quiz_window.protocol("WM_DELETE_WINDOW", lambda: close_window(quiz_window))
    window_stack.append(quiz_window)


def open_difficulty_selection(quiz_type):
    close_previous_window()
    ...
    title_label = tk.Label(difficulty_window, text="Select Difficulty", font=("Helvetica", 20, "bold"), bg="white")
    title_label.pack(pady=20)

    button_font = ("Helvetica", 16, "bold")

    easy_button = tk.Button(difficulty_window, text="Easy", width=22, height=2, bg="green", font=button_font, command=lambda: start_quiz(quiz_type, "easy"))
    easy_button.pack(pady=15)

    challenging_button = tk.Button(difficulty_window, text="Challenging", width=22, height=2, bg="yellow", font=button_font, command=lambda: start_quiz(quiz_type, "challenging"))
    challenging_button.pack(pady=15)

    hard_button = tk.Button(difficulty_window, text="Hard", width=22, height=2, bg="red", font=button_font, command=lambda: start_quiz(quiz_type, "hard"))
    hard_button.pack(pady=15)

    difficulty_window.protocol("WM_DELETE_WINDOW", lambda: close_window(difficulty_window))
    window_stack.append(difficulty_window)


def start_quiz(quiz_type, difficulty):
    """Starts the quiz with the selected difficulty."""
    if quiz_type == "Whose This?":
        open_whose_this_quiz(difficulty)
    elif quiz_type == "Who Can Do That?":
        open_who_can_do_that_quiz(difficulty)


def open_whose_this_quiz(difficulty):
    """Opens the 'Whose This?' quiz window."""
    global window_stack
    close_previous_window()
...
def open_who_can_do_that_quiz(difficulty):
    """Opens the 'Who Can Do That?' quiz window."""
    global window_stack
    close_previous_window()
    ...
    if difficulty == "easy":
        questions = random.sample(easy_questions, 10)
    elif difficulty == "challenging":
        questions = random.sample(challenging_questions, 10)
...
def export_results():
    """Exports quiz results to a text file."""
    global quiz_results
...
def open_history():
    """Opens the quiz results history window."""
    global window_stack, quiz_results
    close_previous_window()

    history_window = tk.Toplevel()
    history_window.title("Quiz Results History")
    history_window.geometry(WINDOW_SIZE)
    history_window.configure(bg=LABEL_BG_COLOR)

    title_label = tk.Label(history_window, text="Quiz Results History", font=TITLE_FONT, bg=LABEL_BG_COLOR)
    title_label.pack(pady=20)

    results_frame = tk.Frame(history_window, bg=LABEL_BG_COLOR)
    results_frame.pack(fill=tk.BOTH, expand=True)

    results_listbox = tk.Listbox(results_frame, font=TEXT_FONT)
    results_listbox.pack(fill=tk.BOTH, expand=True)

    for result in quiz_results:
        results_listbox.insert(tk.END, f"Quiz Type: {result['type']}, Score: {result['score']}")

    export_button = tk.Button(history_window, text="Export Results", font=TEXT_FONT, command=export_results)
    export_button.pack(pady=20)

    close_button = tk.Button(history_window, text="Close", font=TEXT_FONT, command=lambda: close_window(history_window))
    close_button.pack(pady=10)

    history_window.protocol("WM_DELETE_WINDOW", lambda: close_window(history_window))
    window_stack.append(history_window)
...
def open_help():
    ...
    Enjoy learning!
    """
    help_label = tk.Label(help_window, text=help_text, justify=tk.LEFT, bg=LABEL_BG_COLOR)
    help_label.pack(padx=20, pady=10)

    close_button = tk.Button(help_window, text="Close", command=lambda: close_window(help_window))
    close_button.pack(pady=10)

    help_window.protocol("WM_DELETE_WINDOW", lambda: close_window(help_window))
    window_stack.append(help_window)
...
title_label = tk.Label(root, text="Whose that? - Animal Quiz", font=TITLE_FONT, fg="black", bg=None)
title_label.place(relx=0.1, rely=0.05, anchor=tk.W)

# Adding the welcome text box in the middle
welcome_text = ("Welcome to the animal learning and quiz program! If you're confused feel free to click the "
                "help button! But other than that explore the program and have fun!")
...
learn_button = tk.Button(root, text="Learn", width=125, height=75, command=open_learn, fg="black",
                         image=banana_photo, compound="center", borderwidth=0)
learn_button['font'] = BUTTON_FONT
learn_button.place(relx=0.3, rely=0.925, anchor=tk.CENTER)

quiz_button = tk.Button(root, text="Quiz", width=125, height=75, command=open_quiz, fg="black",
                        image=banana_photo, compound="center", borderwidth=0)
quiz_button['font'] = BUTTON_FONT
quiz_button.place(relx=0.5, rely=0.925, anchor=tk.CENTER)

history_button = tk.Button(root, text="History", width=125, height=75, command=open_history, fg="black",
                           image=banana_photo, compound="center", borderwidth=0, state=tk.DISABLED)
history_button['font'] = BUTTON_FONT

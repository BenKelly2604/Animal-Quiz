...
window_stack = []
window_positions = {}  # Tracks positions of all windows by ID
quiz_results = []
fixed_window_position = (100, 100)  # Default position for all windows
...
animal_list = sorted(["Alpaca", "Ant", "Bat", "Bee", "Crocodile", "Deer", "Cheetah", "Duck",
                      ...
                      "Red Panda", "Snake", "Sheep", "Squirrel", "Turkey", "Wolf", "Wombat"])  # List of animal names
animal_descriptions = {
...
animal_descriptions_quiz = {}  # Descriptions of animals for quiz
difficulty_path = {
    "easy": "Images/easy/",
    "challenging": "Images/medium/",
    "hard": "Images/hard/"
}  # Paths for difficulty levels
...
def load_mc_question(quiz_window, idx, score, questions, quiz_type):
    ...
    submit_button = tk.Button(quiz_window, text="Submit", font=TEXT_FONT,
                              command=lambda: display_next_mc_question(quiz_window, idx, score, questions, answer_entry, feedback_label, submit_button, quiz_type))
...
def display_next_mc_question(quiz_window, idx, score, questions, answer_entry, feedback_label, submit_button, quiz_type):
    ...
    if idx < len(questions) - 1:
        submit_button.config(text="Next Question",
                             command=lambda: load_mc_question(quiz_window, idx + 1, score, questions, quiz_type))
    else:
        submit_button.config(text="Show Results",
                             command=lambda: show_results(quiz_window, score, quiz_type))
...
def create_topics_window(title, topics):
    ...
    global window_stack, fixed_window_position
...
def update_window_position(event):
    """Tracks the position of the active window."""
    global fixed_window_position
    if event.widget == root:  # Only track the main window's position
        fixed_window_position = (event.widget.winfo_x(), event.widget.winfo_y())
...
def open_who_can_do_that_quiz(difficulty):
    ...
    # Pass the quiz type to load_mc_question
    quiz_type = "Who Can Do That?"
    load_mc_question(quiz_window, 0, 0, questions, quiz_type)
...
def export_results():
    ...
    with open("quiz_results.txt", "w") as f:
        for result in quiz_results:
            f.write(f"Quiz Type: {result['type']}, Score: {result['score']}/10\n")
...
def open_history():
    ...
    for result in quiz_results:
        results_listbox.insert(tk.END, f"Quiz Type: {result['type']}, Score: {result['score']}/10")
...
def open_help():
    ...
    help_window.geometry(WINDOW_SIZE)
...
learn_button = tk.Button(root, text="Learn", width=125, height=75, command=open_learn, fg="black", image=banana_photo, compound="center", borderwidth=0)
learn_button['font'] = BUTTON_FONT
learn_button.place(relx=0.3, rely=0.925, anchor=tk.CENTER)

quiz_button = tk.Button(root, text="Quiz", width=125, height=75, command=open_quiz, fg="black", image=banana_photo, compound="center", borderwidth=0)
quiz_button['font'] = BUTTON_FONT
quiz_button.place(relx=0.5, rely=0.925, anchor=tk.CENTER)

history_button = tk.Button(root, text="History", width=125, height=75, command=open_history, fg="black", image=banana_photo, compound="center", borderwidth=0, state=tk.DISABLED)
